...
path_hunjam_sources="${path_tmp}"/hunjam_sources.txt
...
rm "${path_hunjam_sources}"         > /dev/null 2>&1
...
while IFS= read -r line; do
  # Skip empty lines
  if [ -n "$line" ]; then
    echo "    $line" | sed 's|\./|\${MY_PROJ_ROOT}/|g' >> "${path_hunjam_sources}"
  fi
done < "${datadir}"/__HUNJAM_SOURCES__.txt
...
list_hunjam_sources=$(cat "${path_hunjam_sources}" | tr '\n' ';')
...
cat "${datadir}/CMakeLists.txt.model" \
    ...
    | sed "s|__HUNJAM_SOURCES__|$list_hunjam_sources|g" \
    | tr ';' '\n' \
    > "${projectfile}"
